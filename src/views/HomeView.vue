...
<template>
  <div class="container mx-auto h-screen py-10 px-4 sm:px-6 md:px-10 flex">
    <div class="w-full lg:w-2/4">
      ...
      <!-- Card container -->
      <div
        class="grid grid-cols-2 md:grid-cols-3 gap-x-5 sm:gap-x-10 gap-y-20 py-14">
        <div
          v-for="_ in 12"
          v-if="isLoading"
          data-testid="skeleton"
          class="p-5 rounded-xl bg-slate-100 h-48 flex flex-col gap-4">
        ...
        <Card
          ...
          :types="pokemon.types"
          @click="showDetail(pokemon)"></Card>
      </div>
      <Pagination></Pagination>
    </div>
    <div
      v-if="pokemonStore.loadingState"
      class="fixed bottom-5 right-5 bg-slate-100 shadow-lg p-5 rounded-xl">
