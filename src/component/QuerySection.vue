...
watch(queryText, (value) => pokemonStore.filterPokemon(value, sortText.value), {
  deep: true,
});
...
<template>
  ...
  <div class="px-5 py-3 bg-white rounded-lg shadow-lg flex gap-5">
    <input
      class="flex-1 text-l px-2 outline-none w-4/5"
      type="text"
      placeholder="Search your pokemon"
      v-model="queryText.searchText"
      @keyup="pokemonStore.searchPokemon(queryText.searchText)" />
    <button class="bg-[#ff5251] rounded-lg p-2 shadow-3xl shadow-[#ff5251]">
      <div
        class="w-9 h-9 border-8 border-white rounded-full flex justify-center items-center">
  ...
  <div class="my-5 p-5 flex flex-wrap justify-between items-center gap-4">
    ...
    <div class="flex gap-5 items-center">
      ...
      <input
        class="text-sm font-bold w-full sm:w-24 p-3 text-center bg-transparent border rounded-lg"
        type="number"
        min="1"
        :max="pokemonStore.totalPage"
        v-model="gotoPageIndex" />
    </div>
  </div>
  <div class="flex p-5 gap-5 justify-between items-start">
    <div class="flex flex-wrap justify-start gap-5">
      <div
        class="bg-white min-w-[170px] px-3 py-2 rounded-lg shadow-sm flex justify-between gap-2 items-center"
        v-for="item in dropdownList">
        <label :for="item.name" class="font-bold">
          {{ item.name }}
        </label>
        <select
          :id="item.name"
          class="text-sm font-bold text-gray-500 rounded-lg outline-none text-center"
          v-model="queryText[item.name]">
          <!-- <option class="text-sm font-bold" selected disabled>{{ item.name }}</option> -->
          <option
            v-for="listData in item.data"
    ...
    <div class="flex items-center justify-end flex-1">
      <button
        class="bg-red-400 border border-red-400 rounded-lg p-2 font-semibold text-white text-sm"
